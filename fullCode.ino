...
// #define gmtOffset_sec "3600" //GMT+1
// #define daylightOffset_sec "3600" //Summer time +1
...
void loop() {
  ...
  if(app.ready()) {
    if (millis() - currentTime >= timeInterval) {
      ...
      if(millis() - currentTime >= historyInterval) {
        String path = "historyData/" + rtc.getUnixTime().toString();
        Database.set<float>(aClient, 
          path, 
          object_t("{\"light\":"+lux.toString()+", 
                  \"temp\":"+temp.toString()+", 
...
void WiFiStart()
{
  ...
  // Optional if your signal is weak:
  // WiFi.setTxPower(WIFI_POWER_19_5dBm);
